...
class DashboardController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        /* =========================
           PEMASUKAN (PEMBAYARAN TAGIHAN)
        ========================= */
        $pemasukan = DB::table('riwayat_pembayaran')
            ->where('user_id', $userId)
            ->where('status', 'Lunas') // 🔥 penting
        ...
        /* =========================
           LABEL MERGE (FIX UTAMA)
        ========================= */
        $labels = collect($pengeluaran->keys())
        ...
        foreach ($labels as $l) {
            ...
            // harian tidak logis per menit → 0
            if ($filter === 'harian') {
                $in[] = 0;
            } else {
                $in[] = $pemasukan[$l]->total ?? 0;
            }
        }
